					...
					$(function () {
						require([ "main" ], function() {
							require(["model/Goods"], function(goods) {
								$("#aj-goodstypes-stamps").find(".stamp-link").each(function() {
									 var params = {
											 plateForm : goods.getPlatForm(),
											 page : 1,
											 goodsTypeId : $(this).parents(".ali").eq(0).data("moreinfo")
									 };
			...
			<div id="goods-type-roll" ng-controller="mainController" ng-cloak>
				...
				<div class="gt-list-fix">
					<ul>
						<li class="li" ng-repeat="item in s.goodsTypeList"
							...
							ng-class="['aj-icon', item.icon]"></em> <span
							title="{{item.name}}" ng-click="changeTo($index)">{{item.shortName}}</span>
						</li>
					</ul>
				</div>
				...
				<div id="gt-list-content">
					<div id="gt-one-block-{{$index}}" class="gt-one"
						...
						<div class="gt-o-top">
							<em class="aj-icon aj-icon-jietijiangli"></em> <span
								class="floor"> {{$index + 1}} </span> <span class="name">
								{{item.shortName}} </span> <a class="more" href="">更多>></a>
						</div>
						<!-- content -->
									...
									<div class="tbkitem-one col-sm-3"
										...
										<div class="border-wrap container-fluid">
											...
											<div class="row shop-line">
												<span
													class="col-sm-8 col-xs-8 max-height-20 overflow-hidden">{{tbkitem.nick}}</span>
												<span
